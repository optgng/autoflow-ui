...
export default function TransactionDetailModal({ tx, onClose }: Props) {
  ...
  // Тема-зависимые классы через HeroUI semantic colors
  const amountColor = isIncome
    ? 'text-success'          // → #00FFA3 dark / #00874A light
    : isExpense
    ? 'text-danger'           // → #FF3366 dark / #DC2626 light
    : 'text-primary';         // → #00E5FF dark / #1A6EF5 light
  ...
  return (
    <ModalPortal>
      ...
      >
        ...
        {/* Карточка */}
        <div
          className="relative glass-card rounded-2xl w-full max-w-md p-6 animate-modal-content"
        ...
        >
          ...
          <div>
            <DetailRow
              ...
              label="Дата операции"
              value={formatDateUI(tx.transaction_date)}
            />
            <DetailRow
